...
Resource Pool은 가상화 환경에서 **물리 자원(CPU/Memory)을 논리적 단위로 분할**하여 각 팀이나 워크로드에 할당하는 메커니즘
...
```
경쟁 상황 예시:
  RP-A (High: 4)  →  전체 자원의 4/6 할당
  RP-B (Low:  2)  →  전체 자원의 2/6 할당
```
 
Note: Share는 **자원 경쟁이 발생할 때만** 적용함. 여유 자원이 충분하면 모두 요청한 만큼 사용
...
```
부모 RP (Reservation: 8GB)
├── 자식 RP-1 (Reservation: 3GB, Expandable: ON)
│   └── 필요 시 부모의 남는 5GB까지 사용 가능 ✅
└── 자식 RP-2 (Reservation: 3GB, Expandable: OFF)
    └── 3GB 초과 사용 불가 ❌
```

<img src="./Images/RP1.png" width="50%">
<img src="./Images/RP2.png" width="50%">
<img src="./Images/RP3.png" width="50%">
...
```
예시 — Host 정책 (허용 호스트 장애: 1)
  Host-1 장애 발생
  → HA가 Host-2, Host-3의 예약된 자원으로 VM 재시작 ✅

예시 — Percent 정책 (CPU 25%, Memory 25% 예약)
  클러스터 총 자원의 25%는 항상 장애 복구용으로 확보
```
